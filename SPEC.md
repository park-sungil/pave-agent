...
| LLM (dev) | Anthropic Claude (Sonnet/Haiku) |
| 프레임워크 | LangGraph |
| DB 드라이버 | python-oracledb (Thick 모드, live) / SQLite (dev) |
...
- DDL-02: LLM — 사내 모델 (prod) / Anthropic Claude (dev)
...
- DDL-07: DB 접속 — Oracle Thick 모드 + TNS DSN (live) / SQLite (dev)
...
├── shared/
│   ├── db.py                        # DB 연결 (Oracle / SQLite)
│   ├── llm.py                       # LLM 클라이언트 (heavy/light 2-tier)
│   └── mock_data.py                 # SQLite mock DB 생성
├── api/
