...
- `shared/db.py`: Oracle/SQLite 자동 전환. `antsdb.` 접두사 제거 + `FETCH FIRST` → `LIMIT` 변환.
- `shared/mock_data.py`: 5개 PDK, 23K행 PPA 데이터 생성. 조건별 modifier로 현실적 분포.
...
### anomaly 이상치 mock 데이터

- SF3(900)의 HVT+125°C 영역에 S_POWER/IDDQ_NA 5배 급등 삽입.
- ULVT+-25°C 영역에 FREQ_GHZ 30% 하락 삽입.
- z-score 기반 탐지로 정확히 포착 확인.
...
`.env`에서 `LLM_PROVIDER=openai_compat`로 변경하고 사내 모델 endpoint 설정.
...
### 3.8 mock 데이터 → 실 데이터 전환 시 확인

- 실 데이터 행 수가 mock(~4,600행/PDK)보다 크면(~13,000행/PDK) FETCH FIRST 제한 확인.
- 실 데이터의 CELL/DS/VTH/CH/WNS 값이 mock과 다를 수 있음. query_builder의 AVG 기본값(INV/ND2/NR2, D1/D4) 확인.
